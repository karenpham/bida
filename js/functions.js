...
function jValidateForm(id, elements, erConfig){
	...
	if(frmObj.length > 0){
		...
		if(typeof(erConfig) == 'undefined'){
			...
			var erLayer = jQuery('<div id="'+ errorConfig.customError +'"><p class="message"></p></div>').appendTo(jQuery(document.body));			
			//create layer
			...
			var erLayer = jQuery(errorConfig.customError);
		}
		...
		function showLayer(el, msg){			
			...
			layer.css({
				'top': coords.top + frmObj.find('[name="'+ el +'"]').outerHeight(),
				'left': coords.left - 2
			});
		...
		function showHideError(el, msg){
			var erEl = jQuery(errorConfig.customError);
			erEl.text(msg);
...
"use strict";
  ...
  var selectnav = function(element,options){
    ...
    function parselist(list){
      ...
      if(level === 1) {
        html = '<div class="select-wrap input-wrap"><i class="icon-angle-down"></i><select class="selectnav" id="'+id(true)+'">' + html + '</select></div>';
      }
